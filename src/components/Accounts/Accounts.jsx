...
const  Accounts = () => {
    const accountsHead = ["ID", "Name", "Account Number"];
  ...
  return (
    <div className='container'>
        <ul className='accounts-table-top table'>
         ...
         <ul className='pages'>
            <div onClick={prev.bind()} className={`arrow ${page === 1 ? 'inactiveArrow' : ""}`} >
                <img src="src/assets/arrowLeft.svg" alt="arrow-previous" />
            </div>

            <div onClick={next.bind()} className={`arrow ${page === totalAccounts ? 'inactiveArrow' : ""}`}>
